...
public class BstSetKTU<E extends Comparable<E>> implements SortedSetADT<E>, Cloneable {
    ...
    public SetADT<E> headSet(E element) 
    {
           ...
           if (element == null)
        {
            throw new IllegalArgumentException("Elementas lygus nuliui");
        }
        SetADT<E> poaibis = new BstSetKTU<>();

        IteratorKTU iterator;
        iterator = new IteratorKTU(true);
        
        while(iterator.hasNext())
        {
            E next = iterator.next();
            int comp = c.compare(element, next);
            if(comp > 0){
               poaibis.add(next);
    ...
    public SetADT<E> tailSet(E element) {
        ...
        {
            E next = iterator.next();
            int comp = c.compare(next, element);
            if(comp >= 0){
    ...
    public  int height()
    {
        if(root == null)
        {
          return 0;
            ...
            return Math.max(heights(root.left), heights(root.right));
     

    }
     private int heights (BstNode<E> node)
    {
        if(node==null)
        {
            return 0;
        ...
        return Math.max(heights(node.left), heights(node.right))+1;
